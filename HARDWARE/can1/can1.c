...
void get_moto_measure(moto_measure_t *ptr, CanRxMsg* hcan, u8 i)
{
		ptr->last_angle = ptr->angle;
		ptr->angle = (uint16_t)(hcan->Data[0]<<8 | hcan->Data[1]) ;
		ptr->real_current  = (int16_t)(hcan->Data[2]<<8 | hcan->Data[3]);
        ...
        if((rx_message.IDE == CAN_Id_Standard)&&(rx_message.IDE == CAN_RTR_Data)&&(rx_message.DLC == 8)) //标准帧、数据帧、数据长度为8
        {
            ...
            else if(rx_message.StdId == 0x4C)
            {
                ...
                Real_Ctl2_Value[3] = rx_message.Data[1];
						...
						if(rx_message.StdId == CAN_3510Moto1_ID)
						{
								get_moto_measure(&moto_chassis[0],&rx_message,0);													
						}
						
						else if(rx_message.StdId == CAN_3510Moto2_ID)
						{
								get_moto_measure(&moto_chassis[1],&rx_message,1);		
						}
						
						else if(rx_message.StdId == CAN_3510Moto3_ID)
						{
								get_moto_measure(&moto_chassis[2],&rx_message,2);									
						}
						
						else if(rx_message.StdId == CAN_3510Moto4_ID)
						{
								get_moto_measure(&moto_chassis[3],&rx_message,3);									
						}
...
void CAN_M3508_Current(unsigned short ID, short Temp_Current_1, short Temp_Current_2, short Temp_Current_3, short Temp_Current_4)
{
    ...
    tx_message.Data[0] = (unsigned char)((Temp_Current_1>>8));
    tx_message.Data[1] = (unsigned char)(Temp_Current_1);
    tx_message.Data[2] = (unsigned char)((Temp_Current_2>>8));
    tx_message.Data[3] = (unsigned char)(Temp_Current_2);
    tx_message.Data[4] = (unsigned char)((Temp_Current_3>>8));
    tx_message.Data[5] = (unsigned char)(Temp_Current_3);
    tx_message.Data[6] = (unsigned char)((Temp_Current_4>>8));
    tx_message.Data[7] = (unsigned char)(Temp_Current_4);
