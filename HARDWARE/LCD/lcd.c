...
//返回：无
//==============================================================
void LED_PrintValueC(u16 x, u16 y, s16 data, u8 size)
{
	...
	j = (data%100)/10;
	k = data%10;
	LCD_ShowChar(x+size/2,y,i+'0', size,mode);
	LCD_ShowChar(x+size,y,j+'0', size,mode);
	LCD_ShowChar(x+size+size/2,y,k+'0', size,mode);	
	
}


 void LED_PrintValueFP(u16 x, u16 y, int data, u8 size, u8 num)
 {
 	u8 m,i,j,k,mode=0;  	
	...
	switch(num)
	{
		case 1:  	LCD_ShowChar(x+size/2,y,k+'0', size,mode);
				break;
		case 2:  LCD_ShowChar(x+size/2,y,j+'0', size,mode);
		LCD_ShowChar(x+size,y,k+'0', size,mode);
				break;
		case 3:	 LCD_ShowChar(x+size/2,y,i+'0', size,mode);
		LCD_ShowChar(x+size,y,j+'0', size,mode);
		LCD_ShowChar(x+size*3/2,y,k+'0', size,mode);
				break;
		case 4: 	LCD_ShowChar(x+size/2,y,m+'0', size,mode);
		LCD_ShowChar(x+size,y,i+'0', size,mode);
		LCD_ShowChar(x+size*3/2,y,j+'0', size,mode);
		LCD_ShowChar(x+size*2,y,k+'0', size,mode);
				break;	
	}
 }
//==============================================================
...
//num小数位数最多四位
//==============================================================
 void LED_PrintValueF(u16 x, u16 y, float data, u8 size, u8 num)
 {
 	u8 l,m,i,j,k;  //万千百十个
 	u8 databiti = 6,mode=0; //整数位数
 	int tempdataui = 0;
  	int tempdataii = (int)data; //整数部分
 	long int tempdatalp = (long int)((data - (int)data)*10000); //取小数位后4位
 	
 	//整数部分显示
 	if(data < 0)  LCD_ShowChar(x,y,'-', size,mode);
 	else  LCD_ShowChar(x,y,'+', size,mode);
	...
	k = tempdataui%10;
 	if (l != 0)  //五位
 	{
 		LCD_ShowChar(x+size/2,y,l+'0', size,mode);
 		LCD_ShowChar(x+size,y,m+'0', size,mode);
		LCD_ShowChar(x+size*3/2,y,i+'0', size,mode);
		LCD_ShowChar(x+size*2,y,j+'0', size,mode);
		LCD_ShowChar(x+size*5/2,y,k+'0', size,mode);
 	}
 	else if(m != 0) //四位
 	{
 		databiti = 5;
 		LCD_ShowChar(x+size/2,y,m+'0', size,mode);
		LCD_ShowChar(x+size,y,i+'0', size,mode);
		LCD_ShowChar(x+size*3/2,y,j+'0', size,mode);
		LCD_ShowChar(x+size*2,y,k+'0', size,mode);
		LCD_ShowChar(x+size*5/2,y,' ', size,mode);
 	}
  	else if(i != 0) //三位
  	{
  		databiti = 4;
  	...
  	{
    		databiti = 3;	
			LCD_ShowChar(x+size*3/2,y,' ', size,mode);
  		LCD_ShowChar(x+size/2,y,j+'0', size,mode);
	...
	{
		case 0: break;
		case 1:  LED_PrintValueFP(x + databiti * size/2, y, (int)(tempdatalp / 1000),size,num);break;
		case 2:  LED_PrintValueFP(x + databiti * size/2, y, (int)(tempdatalp / 100),size,num);break;
		case 3:  LED_PrintValueFP(x + databiti * size/2, y, (int)(tempdatalp / 10),size,num);break;
		case 4:  LED_PrintValueFP(x + databiti * size/2, y, (int)(tempdatalp),size,num);break;					
	}
 }
